...
import { Search, Sprout, Clock, Sun, Droplets, Leaf, Plus } from 'lucide-react';
...
export function AddOtherCropDialog({ isOpen, onClose, language }: Props) {
  ...
  const handleAddCrop = async () => {
    ...
    try {
      ...
      const harvestDate = addDays(sDate, selectedCrop.duration_days || 90);
      ...
      const calendarInserts = events.map(e => ({
        user_id: user.id,
        crop_name: selectedCrop.name,
        event_type: e.type,
              ...
              <CardContent className="p-4">
                ...
                {selectedCrop.water_needs && (
                  <p className="text-sm text-muted-foreground mt-2">
                    💧 {language === 'hi' ? 'पानी:' : language === 'mr' ? 'पाणी:' : 'Water:'} {selectedCrop.water_needs}
                  </p>
                )}
                {selectedCrop.profit_potential && (
                  <p className="text-sm text-muted-foreground">
                    📈 {language === 'hi' ? 'लाभ:' : language === 'mr' ? 'नफा:' : 'Profit:'} {selectedCrop.profit_potential}
                  </p>
                )}
              </CardContent>
            </Card>
